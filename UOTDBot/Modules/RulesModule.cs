...
public sealed class RulesModule : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task Rules()
    {
        await RespondAsync(embed: new EmbedBuilder()
            ...
            .AddField("Usage of transformation", "If all the rules above are met and the map has transformation gates, there's one additional check: if the current WR uses non-Stadium car for more than a certain threshold. The official UOTD threshold is 1%, but the default value to report it is 50%. What gets reported can be adjusted with `/report threshold`.")
